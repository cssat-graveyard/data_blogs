...
zips_dat <- sqlQuery(cn, "select
                          ...
                          where isnull(rp.discharge_dt, '3999-01-01') >= '2014-01-01'
...
dist_d <- data.frame(zip=rep(NA, n), dist=rep(NA, n))
...
med <- as.numeric(names(table(summary(dist_by_zip$avg)))[3])
#assign to zips
...
ggplot(dist_d, aes(x=dist_m)) + 
  ...
  geom_vline(xintercept = 23.95) +
  annotate("text", x = 60, y = .14, label = "Median Distance", family="Frutiger LT Std 45 Light") +
  theme_bw() +
  theme(text=element_text(size=16, family="Frutiger LT Std 45 Light")
        ,axis.text.x=element_text(angle=90))
dev.off()
